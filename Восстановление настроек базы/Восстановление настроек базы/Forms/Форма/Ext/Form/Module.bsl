...
Процедура ПутьКФайлуСНастройкамиНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если Переключатель = "Каталог" Тогда
		ДиалогВыбораФайла = Новый ДиалогВыбораФайла(РежимДиалогаВыбораФайла.ВыборКаталога);	
		ДиалогВыбораФайла.Заголовок = "Выберите каталог с настройками баз данных";
		ДиалогВыбораФайла.Каталог = ПутьКФайлуСНастройками;	
	Иначе
		...
		Если Переключатель = "Архив" Тогда
			ДиалогВыбораФайла.Фильтр = "Настройки баз данных (*.zip)|*.zip|";
		Иначе
...
Процедура УстановкаНастроекНаСервере(ТекущаяБазаДанных)
	...
	Исключение
		МодульБлокировкаРаботыСВнешнимиРесурсами = Неопределено;
	КонецПопытки; 
		
	ИсправитьОшибкиБазыПриОбновлении();
	НачатьТранзакцию();
	Попытка	
		Если МодульБлокировкаРаботыСВнешнимиРесурсами <> Неопределено 
			И НЕ МодульБлокировкаРаботыСВнешнимиРесурсами.РаботаСВнешнимиРесурсамиЗаблокирована() Тогда
		...
		УстановитьЗаголовокБазы(ТекущаяБазаДанных);
...
Процедура ПодключитьРасширениеБазыДанных(Расширение)
	...
	Если ПодключаемыеРасширения.Количество() Тогда
		...
		Если НЕ Перезаписывать Тогда
			...
			СохранитьДополнительныеПараметрыРасширения(Расширение);
			Возврат;
		КонецЕсли; 
...
&НаСервереБезКонтекста
Процедура СохранитьДополнительныеПараметрыРасширения(НастройкиРасширения)
	Для Каждого Настройка Из ЗначениеПараметра(НастройкиРасширения, "ДополнительныеНастройки", Новый Массив) Цикл
...
Процедура ОбнулитьНомераСообщенийОбменаИРегистрацию(НастройкаОбмена)
	...
	УзелБазыПриемника = ПланыОбмена[ИмяПланаОбмена].НайтиПоКоду(НастройкаОбмена.КодУзлаОбмена);
	...
	УзелТекущегоПлана = ПланыОбмена[ИмяПланаОбмена].НайтиПоКоду(НастройкаОбмена.КодУзлаОбмена); 
	УзелПланаОбъект = УзелТекущегоПлана.ПолучитьОбъект();
	УзелПланаОбъект.НомерПринятого = 0;
	УзелПланаОбъект.НомерОтправленного = 0;
	УзелПланаОбъект.Записать();
	
	ПланыОбмена.УдалитьРегистрациюИзменений(УзелТекущегоПлана);
	
	Если НЕ МетаданныеСуществуют("РегистрСведений.ОбщиеНастройкиУзловИнформационныхБаз") Тогда
		Возврат;
	КонецЕсли;

	Если Метаданные.РегистрыСведений["ОбщиеНастройкиУзловИнформационныхБаз"].Ресурсы.Найти("НастройкаЗавершена") = Неопределено Тогда
		Возврат;
	КонецЕсли;
...
Процедура ОтключитьРегламентныеЗадания(ПараметрыБазыДанных)
	...
	Если НЕ ЗначениеПараметра(ПараметрыБазыДанных, "ОтключатьФоновыеЗадания", Ложь) Тогда
		Возврат;
	КонецЕсли; 
	...
	Для Каждого РЗадание Из СписокРегламентныхЗаданий Цикл
		Если СписокДопустимыхФоновыхЗаданий.Найти(РЗадание.Метаданные.Имя) <> Неопределено Тогда
			Если НЕ РЗадание.Использование Тогда
				РЗадание.Использование = Истина;
				РЗадание.Записать();
			КонецЕсли; 			
		ИначеЕсли РЗадание.Использование Тогда 
...
&НаСервереБезКонтекста
Процедура УстановитьЗаголовокБазы(ПараметрыБазыДанных)
...
Функция НастройкиБазДанныхИзФайла()
	...
	Если НЕ ФайлСуществует(ПутьКФайлуСНастройками) Тогда
		ВызватьИсключение "Не найден файл с настройками баз данных";
	КонецЕсли;
	
	НастройкиБаз = РаспаковатьНастройкиБазПоПереключателю(ПутьКФайлуСНастройками, Переключатель);
	
	ДополнительныеПараметры = ЗначениеПараметра(НастройкиБаз, "ДополнительныеПараметры", Новый Структура);
	ДополнительныеПараметры.Вставить("ТекущийКаталог", 	ТекущийКаталог);
	ДополнительныеПараметры.Вставить("КаталогНастроек", КаталогНастроек(ПутьКФайлуСНастройками));
	...
	БазыДанных = ЗначениеПараметра(НастройкиБаз, "БазыДанных", Новый Массив);
...
&НаКлиенте
Функция РаспаковатьНастройкиБазПоПереключателю(ПутьКФайлу, Режим)
	
	Если Режим = "Файл" Тогда
		ДанныеНастроекБаз = ПрочитатьФайлНастроек(ПутьКФайлу);	
	ИначеЕсли Режим = "Каталог" Тогда 	
		ДанныеНастроекБаз = ДанныеФайлаНастроекИзКаталога(ПутьКФайлу);
	ИначеЕсли Режим = "Архив" Тогда 
		КудаВыгружатьАрхив = ОбъединитьПути(КаталогВременныхФайлов(), СтрЗаменить(Новый УникальныйИдентификатор, "-", ""));
		СоздатьКаталог(КудаВыгружатьАрхив);
		Архив = Новый ЧтениеZipФайла(ПутьКФайлу);		
		Архив.ИзвлечьВсе(КудаВыгружатьАрхив, РежимВосстановленияПутейФайловZIP.Восстанавливать);
	    ДанныеНастроекБаз = ДанныеФайлаНастроекИзКаталога(КудаВыгружатьАрхив);
		УдалитьФайлы(КудаВыгружатьАрхив);
	КонецЕсли; 
	
	Возврат ДанныеНастроекБаз;
	
КонецФункции

&НаКлиенте
Функция ПрочитатьФайлНастроек(ПутьКФайлу)
	ЧтениеJSON = Новый ЧтениеJSON();
	ЧтениеJSON.ОткрытьФайл(ПутьКФайлу);
	ПараметрыДляСохранения = ПрочитатьJSON(ЧтениеJSON);
	ЧтениеJSON.Закрыть();
	Возврат ПараметрыДляСохранения;	
КонецФункции

&НаКлиенте
Функция ДанныеФайлаНастроекИзКаталога(КаталогСНастройками)
	
	ФайлыДляВосстановления = НайтиФайлы(КаталогСНастройками, "*.json", Истина);
	
	ДанныеФайлаНастроек = Новый Структура;
	ДанныеФайлаНастроек.Вставить("БазаДанных", Новый Массив);
	ДанныеФайлаНастроек.Вставить("ДополнительныеПараметры", Новый Структура);
	
	Для Каждого Файл Из ФайлыДляВосстановления Цикл
		ДанныеФайлаДляВосстановления = ПрочитатьФайлНастроек(Файл.ПолноеИмя);
		Если Файл.ИмяБезРасширения = "ДополнительныеПараметры"
			ИЛИ Файл.ИмяБезРасширения = "AdditionalSettings" Тогда
			Если ДанныеФайлаДляВосстановления.Свойство("ДополнительныеПараметры") Тогда
				ОбщегоНазначенияКлиентСервер.ДополнитьСтруктуру(ДанныеФайлаНастроек.ДополнительныеПараметры, ДанныеФайлаДляВосстановления.ДополнительныеПараметры); 
			Иначе
				ОбщегоНазначенияКлиентСервер.ДополнитьСтруктуру(ДанныеФайлаНастроек.ДополнительныеПараметры, ДанныеФайлаДляВосстановления); 
			КонецЕсли; 
		Иначе
			Если ТипЗнч(ДанныеФайлаДляВосстановления) = Тип("Структура") Тогда 
				Если ДанныеФайлаДляВосстановления.Свойство("БазыДанных") Тогда
					Для Каждого Элемент Из ДанныеФайлаДляВосстановления Цикл
						Если Элемент.Ключ = "БазыДаных" Тогда
							Если ТипЗнч(Элемент.Значение) = Тип("Массив") Тогда
								ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ДанныеФайлаНастроек.БазыДанных, Элемент.Значение);
							ИначеЕсли ТипЗнч(Элемент.Значение) = Тип("Структура") Тогда 
								ДанныеФайлаНастроек.БазыДанных.Добавить(Элемент.Значение);
							КонецЕсли;
						ИначеЕсли Элемент.Ключ = "ДополнительныеПараметры" Тогда
							ОбщегоНазначенияКлиентСервер.ДополнитьСтруктуру(ДанныеФайлаНастроек.ДополнительныеПараметры, Элемент.Значение, Ложь);
						КонецЕсли; 	
					КонецЦикла; 
				Иначе
					ДанныеФайлаНастроек.БазыДанных.Добавить(ДанныеФайлаДляВосстановления);
				КонецЕсли; 
			КонецЕсли;
		КонецЕсли; 
	КонецЦикла; 
...
&НаКлиенте
Функция КаталогНастроек(ПутьКФайлу)
	Файл = Новый Файл(ПутьКФайлу);
	Если Файл.ЭтоКаталог() Тогда
		Возврат УбратьСлешВКонце(ПутьКФайлу);	
	Иначе
		Возврат УбратьСлешВКонце(Файл.Путь);
	КонецЕсли; 
КонецФункции
...
Процедура ПереключательПриИзменении(Элемент)
	 ОбработатьПереключениеАдреса(ЭтаФорма);
КонецПроцедуры

&НаСервере
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	
	Если НЕ ЗначениеЗаполнено(Переключатель) Тогда
		Переключатель = "Файл";
	КонецЕсли; 
...
&НаКлиентеНаСервереБезКонтекста
Процедура ОбработатьПереключениеАдреса(Форма)
	
	Если НЕ ЗначениеЗаполнено(Форма.ПутьКФайлуСНастройками) Тогда
		Возврат;
	КонецЕсли; 
	
	Файл = Новый Файл(Форма.ПутьКФайлуСНастройками);
	Форма.ПутьКФайлуСНастройками = Лев(Форма.ПутьКФайлуСНастройками, СтрДлина(Форма.ПутьКФайлуСНастройками) - СтрДлина(Файл.Расширение));
	Если Форма.Переключатель = "Файл" Тогда
		Форма.ПутьКФайлуСНастройками = Форма.ПутьКФайлуСНастройками + ".json";	
	ИначеЕсли Форма.Переключатель = "Архив" Тогда 
		Форма.ПутьКФайлуСНастройками =  Форма.ПутьКФайлуСНастройками + ".zip";
	КонецЕсли; 
